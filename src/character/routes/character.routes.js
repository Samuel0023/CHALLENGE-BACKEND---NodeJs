...
const { isValidateName, isValidateMovie } = require('../helpers/validateCharacter');
...
router.post('/create', [
    ...
    validateFields
], CharacterController.createProduct);
//Actualizar - private - (token valid) "put"

router.put('/:id', [
    validateJWT,
    check('id_movie').not().isEmpty(),
    check('id_movie').custom(isValidateMovie),
    validateFields
], CharacterController.updateProduct);
